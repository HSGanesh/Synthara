...
function SidebarHistoryItem({ item, onSelect, onDelete, isDeleting }) {
...
export default function Chat() {
  ...
  const handleSelectHistory = (item) => {
    ...
    setMessages([
      { role: 'assistant', content: `Restored conversation from **${item.collection}**.`, sources: [] },
  ...
  const handleRepoOverview = async () => {
    ...
    setMessages(prev => [...prev,
      { role: 'user', content: 'Explain this repository', sources: [] }
    ])
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', paddingTop: '56px' }}>
      ...
      <div style={{
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px', alignItems: 'center' }}>
          ...
          <button
            onClick={() => { localStorage.removeItem('synthara_chat'); handleNewChat() }}
            style={{
              background: 'none',
              border: '1px solid var(--border)',
              color: 'var(--text-muted)', cursor: 'pointer',
              fontSize: '10px', fontFamily: 'Space Mono',
              padding: '6px 10px', borderRadius: '6px', letterSpacing: '0.5px',
            }}
          >CLEAR</button>
        </div>
      </div>
